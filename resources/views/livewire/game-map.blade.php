    ...
    <div class="flex-1 relative overflow-hidden p-10 bg-gradient-to-b from-blue-50 to-green-50">
        ...
        <div class="max-w-4xl mx-auto text-center">
            <h2 class="text-5xl font-black text-gray-800 mb-4 animate-fade-in-down">
                Mundo: <span class="text-transparent bg-clip-text bg-gradient-to-r {{ $world === 'sol' ? 'from-purple-600 to-pink-500' : 'from-blue-600 to-cyan-500' }}">{{ $worldName }}</span>
            </h2>
            ...
            <div class="grid grid-cols-2 md:grid-cols-5 gap-10 py-10">
                @foreach($levels as $level)
                    <div class="relative flex flex-col items-center">
                        ...
                        @if(!$loop->last)
                            <div class="hidden md:block absolute top-10 -right-10 w-20 h-2 bg-gray-200 rounded-full z-0 overflow-hidden">
                        ...
                        <!-- Botón de Nivel -->
                        <button wire:click="playLevel({{ $level['level'] }})" 
                            {{ !$level['is_unlocked'] ? 'disabled' : '' }}
                        ...
                        @if($level['is_unlocked'])
                            <div class="mt-4 flex gap-1">
                                @for($i = 1; $i <= 3; $i++)
                                    <span class="text-2xl {{ $i <= $level['stars'] ? 'text-yellow-400 animate-pulse' : 'text-gray-300' }}">
                                        {{ $i <= $level['stars'] ? '★' : '☆' }}
                                    </span>
                                @endfor
                            </div>
                        ...
                        @if($level['is_unlocked'] && !$level['is_completed'])
                            <div class="absolute -top-4 -right-2 bg-pink-500 text-white text-[10px] font-black px-2 py-1 rounded-full animate-bounce">
                                ¡TOCA AQUÍ!
                            </div>
                        @endif
                    </div>
            ...
            @if($lastPlayed)
                <div class="mt-12 bg-white/60 backdrop-blur-sm p-6 rounded-[2rem] border-4 border-white inline-flex items-center gap-6 shadow-xl animate-fade-in-up">
                    <div class="bg-yellow-400 w-16 h-16 rounded-full flex items-center justify-center text-3xl shadow-lg border-b-4 border-yellow-600">
                        ⭐
                    </div>
                    <div class="text-left">
                        <p class="text-gray-500 font-bold">Última vez jugaste:</p>
                        <p class="text-xl font-black text-gray-800">
                            Nivel {{ $lastPlayed->level }} ({{ $lastPlayed->world === 'sol' ? 'Clave de Sol' : 'Clave de Fa' }})
                        </p>
                    </div>
